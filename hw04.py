...
ksize = 5     # try something between 5 and 15
sigma = 2.0     # try something between 2.0 and 4.0
...
se_size = 9  # size of your structuring element (morphological operation kernel) -- try something between 5 and 15
...
cv2.imshow("After Morph Ops", res3)
grad = cv2.MORPH_GRADIENT
res3 = cv2.morphologyEx(res3, grad, kernel=se)

se_size = 8
se = np.ones((se_size,se_size), np.uint8)
erode = cv2.MORPH_ERODE
res3 = cv2.morphologyEx(res3, erode, kernel=se)
cv2.imshow("Final result",res3)
